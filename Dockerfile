...
# Switch apt mirror (closer to Russia, makes initial process faster):
RUN perl -pi -e"s#archive\.ubuntu\.com#mirror.yandex.ru#g" /etc/apt/sources.list

# Common packages
RUN apt-get update && apt-get install -y \
      build-essential \
      software-properties-common sudo man-db virtualenv \
	  perl perl-doc libterm-readline-perl-perl libterm-readkey-perl libterm-readline-gnu-perl cpanminus ack-grep default-mysql-client-core libmysqlclient-dev libdbi-perl libdbd-mysql libdbd-mysql-perl libdbd-pg-perl libdatetime-perl libmoose-perl libdbix-class-schema-loader-perl libconfig-inifiles-perl \
      tzdata \
      psmisc \
      curl \
      git \
      ...
      vim exuberant-ctags \
      mosh \
      python3 python3-dev python3-pip python3-virtualenv python3-numpy python3-pandas python3-skimage-lib \
	  ruby ruby-dev
...
# TODO: not tested for Ubuntu 20.04:
